...
export function projectReducer(state, action) {
    switch (action.type) {
        ...
        case PROJECT_ACTIONS.ADD_GENERATED_IMAGE_SUCCESS:
            return {
                ...
                scenes: state.scenes.map((scene) => {
                    ...
                    const selectedGeneratedImageUrl =
                        newGeneratedImage &&
                        newGeneratedImage.gcsUrls &&
                        newGeneratedImage.gcsUrls.length > 0
                            ? newGeneratedImage.gcsUrls[newGeneratedImage.selectedImageIdx] ||
                              newGeneratedImage.gcsUrls[0]
                            : null;
                ...
                scenes: state.scenes.map((scene) => {
                    ...
                    const selectedGeneratedImageUrl =
                        ...
                        selectedGeneratedImage.gcsUrls.length > 0
                            ? selectedGeneratedImage.gcsUrls[
                                  selectedGeneratedImage.selectedImageIdx
                              ] || selectedGeneratedImage.gcsUrls[0]
                            : null;
        ...
        case PROJECT_ACTIONS.REMOVE_ELEMENT_IMAGE_SUCCESS:
            return {
                ...state,
                elementImages: state.elementImages.filter(
                    (img) => img.id !== action.payload.elementImageId
                ),
            };
        ...
        case PROJECT_ACTIONS.UPDATE_ELEMENT_IMAGE_SUCCESS:
            return {
                ...
                elementImages: state.elementImages.map((img) =>
                    img.id === action.payload.elementImage.id ? action.payload.elementImage : img
                ),
            };
        ...
        case PROJECT_ACTIONS.ADD_SCENE_SUCCESS:
            ...
            const updatedScenes = [...state.scenes, newScene].sort(
            ...
            return {
                ...
                scenes: state.scenes.map((scene) => {
                    ...
                    if (scene.selectedGeneratedImageId === action.payload.imageId) {
                        ...
                        updatedSelectedGeneratedImage =
                            updatedGeneratedImage &&
                            updatedGeneratedImage.gcsUrls &&
                            updatedGeneratedImage.gcsUrls.length > 0
                                ? updatedGeneratedImage.gcsUrls[action.payload.selectedIndex] ||
                                  updatedGeneratedImage.gcsUrls[0]
                                : null;
                    }
        ...
        case PROJECT_ACTIONS.REMOVE_SCENE_CLIP_SUCCESS:
            return {
                ...state,
                scenes: state.scenes.map((scene) => {
                    if (scene.id !== action.payload.sceneId) {
                    ...
                    // Remove the clip from the scene's clips array
                    const updatedSceneClips = scene.sceneClips.filter(
                        (clip) => clip.id !== action.payload.clipId
                    );

                    // If this was the selected clip, clear selection
                    const updates = {
                        sceneClips: updatedSceneClips,
                    };

                    if (action.payload.wasSelected) {
                        updates.selectedSceneClip = null;
                        updates.selectedSceneClipId = null;
                    }
